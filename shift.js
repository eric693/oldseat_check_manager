...
function autoFillShiftTime(shiftType) {
    const times = {
        // ========== 廚房班別（8個） ==========
        '廚房A班': ['11:00', '20:00'],  // 9小時
        '廚房B班': ['11:00', '23:00'],  // 12小時
        '廚房C班': ['14:00', '23:00'],  // 9小時（下午班）
        '廚房D班': ['17:00', '23:00'],  // 6小時（晚班）
        '廚房E班': ['11:00', '15:00'],  // 4小時（短班）
        '廚房F班': ['12:00', '21:00'],  // 9小時
        '廚房G班': ['17:00', '22:00'],  // 5小時（晚班短）
        '廚房H班': ['12:00', '23:00'],  // 11小時
        
        // ========== 外場班別（10個） ==========
        '外場A1班': ['11:00', '20:00'],  // 9小時
        '外場A2班': ['11:00', '16:00'],  // 5小時
        '外場A3班': ['11:00', '17:00'],  // 6小時
        '外場A4班': ['11:00', '00:00'],  // 13小時（跨日）
        '外場A5班': ['12:00', '00:00'],  // 12小時（跨日）
        '外場B1班': ['15:00', '00:00'],  // 9小時（跨日）
        '外場B2班': ['18:00', '00:00'],  // 6小時（跨日）
        '外場B3班': ['19:00', '00:00'],  // 5小時（跨日）
        '外場B4班': ['18:30', '22:00'],  // 3.5小時
        '外場B5班': ['18:00', '23:00'],  // 5小時
...
function downloadTemplate() {
    const template = '員工ID,員工姓名,日期,班別,上班時間,下班時間,地點,備註\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-01,廚房A班,11:00,20:00,總公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-02,廚房B班,11:00,23:00,分公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-03,廚房C班,14:00,23:00,總公司,下午班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-04,廚房D班,17:00,23:00,分公司,晚班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-05,廚房E班,11:00,15:00,總公司,短班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-06,廚房F班,12:00,21:00,分公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-07,廚房G班,17:00,22:00,總公司,晚班短\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-08,廚房H班,12:00,23:00,分公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-09,外場A1班,11:00,20:00,總公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-10,外場A2班,11:00,16:00,分公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-11,外場A3班,11:00,17:00,總公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-12,外場A4班,11:00,00:00,分公司,跨日班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-13,外場A5班,12:00,00:00,總公司,跨日班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-14,外場B1班,15:00,00:00,分公司,跨日班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-15,外場B2班,18:00,00:00,總公司,跨日班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-16,外場B3班,19:00,00:00,分公司,跨日班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-17,外場B4班,18:30,22:00,總公司,短班\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-18,外場B5班,18:00,23:00,分公司,\n' +
                    'Udf67b768920d99e5754478ff8c14f13f,洪培瑜Eric,2026-02-19,年假(特休),00:00,00:00,總公司,特別休假\n' +
...
// ========== 班別定義（對應 getShiftTypeBadge）==========

const KITCHEN_TYPES = ['廚房A班','廚房B班','廚房C班','廚房D班','廚房E班','廚房F班','廚房G班','廚房H班','廚房I班'];
const FLOOR_TYPES   = ['外場A1班','外場A2班','外場A3班','外場A4班','外場B1班','外場B2班','外場B3班','外場B4班'];
...
function displayMonthlyStatsKitchen(shifts) {
    ...
    KITCHEN_TYPES.forEach(type => {
        stats[type] = kitchenShifts.filter(s => s.shiftType === type).length;
    });
    ...
    statsGrid.innerHTML = `
        ...
        <div class="stat-card">
            ...
            <div class="stat-value" style="color:${KITCHEN_COLORS[type]};">${stats[type]}</div>
        </div>`).join('')}
    `;
}
...
function displayMonthlyStatsFloor(shifts) {
    ...
    FLOOR_TYPES.forEach(type => {
        stats[type] = floorShifts.filter(s => s.shiftType === type).length;
    });
    ...
    statsGrid.innerHTML = `
        ...
        <div class="stat-card">
            <div class="stat-label">${type}</div>
            <div class="stat-value" style="color:${FLOOR_COLORS[type]};">${stats[type]}</div>
        </div>`).join('')}
    `;
}
            ...
            <div class="calendar-day ${otherMonthClass} ${isToday ? 'today' : ''} ${hasShifts ? 'has-shifts' : ''}">
                ...
                <div class="day-shifts">
                    ${dayShifts.slice(0, 10).map(shift => {
                        ...
                        // getShiftTypeBadge 對應的 CSS class 透過 getShiftClass() 取得
                        return `
...
function displayShiftDistributionKitchen(shifts) {
    ...
    const employeeStats = {};
    // 對應 getShiftTypeBadge 廚房班別（A~I）
    const typeStats = {};
    KITCHEN_TYPES.forEach(t => { typeStats[t] = 0; });
    ...
    container.innerHTML = buildDistributionHTML(
        employeeStats, typeStats, KITCHEN_COLORS, '#ff9800', '廚房員工排班分布', '廚房班別分布'
    );
}
...
function displayShiftDistributionFloor(shifts) {
    ...
    const employeeStats = {};
    // 對應 getShiftTypeBadge 外場班別（A1~A4、B1~B4）
    const typeStats = {};
    FLOOR_TYPES.forEach(t => { typeStats[t] = 0; });
    ...
    container.innerHTML = buildDistributionHTML(
        employeeStats, typeStats, FLOOR_COLORS, '#2196f3', '外場員工排班分布', '外場班別分布'
    );
}
...
function buildDistributionHTML(employeeStats, typeStats, typeColors, barColor, empTitle, typeTitle) {
    ...
    const typeCards = Object.entries(typeStats).map(([type, count]) => {
        ...
        // getShiftClass() 對應到 getShiftTypeBadge 相同的 CSS class 命名
        return `
            <div class="shift-type-stat">
                <div class="shift-type-stat-header">
                    <span class="shift-type-label ${getShiftClass(type)}">${type}</span>
